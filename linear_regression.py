...
def batch_gradient_descent(
    ...
    *,
    ...
    for current_epoch_number in range(max_epochs):
        ...
        ###################################################################
        gradient = X_train.T.dot(X_train).dot(w) - X_train.T.dot(y_train)
        w -= eta * gradient
        ###################################################################
        #                        END OF YOUR CODE                         #
        ###################################################################
...
def closed_form(
    ...
    *,
    ...
    ###################################################################
    M = X_train.shape[1]
    w = np.linalg.inv(reg*np.identity(M) + X_train.T.dot(X_train)).dot(X_train.T).dot(y_train)
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
...
def closed_form_locally_weighted(
    ...
    R = np.diag((r_train))
    X_T = X_train.T
    w = np.linalg.inv(X_T.dot(R).dot(X_train)).dot(X_T.dot(R).dot(y_train))
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
    assert w is not None, "You are asked to update w properly"
...
def compute_y_space(
    ...
    for idx, x_point in enumerate(x_space):
        ...
        #########################################################################
        r = np.exp(-1 * np.square(x_point - x_train) / (2 * tau**2))
        w = closed_form_locally_weighted(X_train, y_train, r)
        phi_x = np.array([x_point**i for i in range(X_train.shape[1])])
        y_space[idx] = phi_x.dot(w)
        #########################################################################
        #                          END OF YOUR CODE                             #
        #########################################################################
